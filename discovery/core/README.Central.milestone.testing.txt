...
The forthcoming (current) milestone release URL is 

	http://download.jboss.org/jbosstools/discovery/development/kepler/jbosstools-directory.xml

The stable branch (upcoming milestone) URL will be 
...
The same as used for all milestone releases of JBoss Tools; should you want to pull the list of available updates from a NEWER directory file
(eg., during a QE cycle when there's a milestone candidate available, but the published milestone site still contains the PREVIOUS milestone), 
you can do so using a commmandline flag.
